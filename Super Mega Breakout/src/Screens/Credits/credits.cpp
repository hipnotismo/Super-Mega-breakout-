...
namespace credits {
	...
	void creditsDraw() {
		...
		DrawText("Made by: Martin Concetti", static_cast<int>(GetScreenWidth() /2), 0, 30, SKYBLUE);
		DrawText("Player ball collision from raylib example, bricks from Matias Karplus", static_cast<int>(GetScreenWidth() / 8), static_cast<int>(GetScreenHeight()) / 19, 20, SKYBLUE);
		DrawText("Using raylib", static_cast<int>(GetScreenWidth() /2), static_cast<int>(GetScreenHeight()) / 10, 30, SKYBLUE);
		DrawText("V0.10", static_cast<int>(GetScreenWidth() /2), static_cast<int>(GetScreenHeight()) / 7, 30, SKYBLUE);
